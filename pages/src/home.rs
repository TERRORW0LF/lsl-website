                            ...
                            <div class="column">
                              <div class="row">
                                {if let Some(title) = &act.title_old {
                                  EitherOf3::A(
                                    view! { <h5 class=format!("{} color", title.to_string())>{title.to_string()}</h5> },
                                  )
                                } else if let Some(rank) = act.rank_old {
                                  EitherOf3::B(
                                    view! {
                                      <h5 class=format!("rank-{rank} color")>{format!("#{}", rank.to_string())}</h5>
                                    },
                                  )
                                } else {
                                  EitherOf3::C(view! { <h5>"User Joined"</h5> })
                                }} <h6>{if act.title_old.is_some() || act.rank_old.is_some() { "to" } else { "" }}</h6>
                              ...
                              <h6>"for " {act.user.username.clone()}</h6>
                            </div>
                            <div class="column">
                              <p>
                                {if let Some(ref l) = act.layout {
                              ...
                              <p>
                                {if let Some(ref c) = act.category {
                                  Either::Left(c.clone())
                                } else {
                                  Either::Right(view! { <br /> })
                                }}
                              ...
                              <p>
                                {if diff.num_hours() == 0 {
                                  format!("{}m {}s", diff.num_minutes(), diff.num_seconds() - diff.num_minutes() * 60)
                                } else if diff.num_days() == 0 {
                                  format!("{}h {}m", diff.num_hours(), diff.num_minutes() - diff.num_hours() * 60)
                                } else {
                                  format!("{}d {}h", diff.num_days(), diff.num_hours() - diff.num_days() * 24)
                                }} " ago"
                              </p>
                            </div>
                          </div>
                  ...
                  .and_then(|runs| {
                    runs
                      ...
                      .map(|run| {
                        ...
                        view! {
                          <div class="row">
                            <a href=run.proof.clone() target="_blank" class="play"></a>
                            <div class="column">
                              <a href=format!("/leaderboard/map/{}", run.section.id)>
                                <h5>{run.section.map.clone()}</h5>
                              </a>
                              <h6>{run.time.to_string()} " sec by " {run.user.username.clone()}</h6>
                            </div>
                            <div class="column">
                              <p>"Layout " {run.section.layout.clone()}</p>
                              <p>{run.section.category.clone()}</p>
                              <p>
                                {if diff.num_hours() == 0 {
                                  format!("{}m {}s", diff.num_minutes(), diff.num_seconds() - diff.num_minutes() * 60)
                                } else if diff.num_days() == 0 {
            ...
            <div class="row narrow">
              <div class="ranks">
                ...
                {move || {
                  potd
                    .and_then(|u| {
                      let mut ranks = u.ranks.clone();
                      ranks.sort_by_key(|r| r.layout.clone());
                      ranks
                        .iter()
                        .filter(|r| { r.patch == String::from("2.13") && r.category == Some(String::from("Standard")) })
                        .map(|r| {
                          view! {
                            <div class="row">
                              <div class="column">
                  ...
                  potd
                    .and_then(|u| {
                      ...
                      ranks
                        .iter()
                        .filter(|r| {
                          r.patch == String::from("2.13") && r.category == Some(String::from("Gravspeed"))
                        })
                        .map(|r| {
                          view! {
                            <div class="row">
