        ...
        <Suspense fallback=|| {
          ...
          {move || {
            user
              .and_then(|u| {
                view! {
                  <h1>{u.username.clone()}</h1>
                  <div class="row">
                    <div>
                      <img class="avatar" src=format!("/cdn/users/{}.jpg", u.pfp) />
                      <p>{u.bio.clone().unwrap_or("This user has no about me.".into())}</p>
                    </div>
                        ...
                        .map(|r| {
                          view! {
                            <TableLine>
                              ...
                              {format!("Layout {}", r.section.layout)} {r.section.category} {r.section.map}
                              <a href=r.proof>"link"</a> {format!("{} sec", r.time.to_string())}
                              <A
                                attr:class="delete"
                                href=move || { format!("{}{}", r.id.to_string(), params.get().to_query_string()) }
                              >
                                <div></div>
                              </A>
                            </TableLine>
